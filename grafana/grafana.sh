...
if ! cube_file_exists "/etc/letsencrypt/live/${cubevar_app_grafana_host}/fullchain.pem" ; then
  certbot certonly --non-interactive --expand --agree-tos --email contact@myplaceonline.com --dns-digitalocean --dns-digitalocean-credentials ~/.digitalocean.ini --dns-digitalocean-propagation-seconds 120 -d ${cubevar_app_grafana_host} || cube_check_return
