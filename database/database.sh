...
if ! cube_has_role "database_backup" ; then
  ...
  if cube_set_file_contents_string "/var/spool/cron/root" "0 0 * * * date >> /var/log/crontab.log 2>&1 && rsync -avr ${cubevar_app_nfs_client_mount} ${cubevar_app_nfs_client_mount_backup} >> /var/log/crontab.log 2>&1" ; then
