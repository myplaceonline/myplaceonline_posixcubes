...
cubevar_app_redirect=$(curl --trace-ascii /dev/stdout -u "admin:${cubevar_app_passwords_haproxy_stats}" -d "s=web56" -d "s=web57" -d "action=ready" -d "b=#4" -w "%{redirect_url}" https://myplaceonline.com:9443/) || cube_check_return
