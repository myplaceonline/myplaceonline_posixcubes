...
time ssh root@${cubevar_app_email_host} "tar czvf ${EMAILFILES} /var/vmail/"
...
time scp root@${cubevar_app_backup_node}:${DBBACKUP} ${OUTPUTDIR} && \
  ...
  echo "gpg --s2k-mode 3 --s2k-count 65536 --force-mdc --cipher-algo AES256 --s2k-digest-algo sha512 -o ${EMAILFILES#/}.pgp --symmetric ${EMAILFILES#/}; rm ${EMAILFILES#/}"
