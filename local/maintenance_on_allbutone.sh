...
cubevar_app_redirect=$(curl --trace-ascii /dev/stdout -u "admin:${cubevar_app_passwords_haproxy_stats}" -d "s=web12" -d "action=maint" -d "b=#4" -w "%{redirect_url}" https://myplaceonline.com:9443/) || cube_check_return
