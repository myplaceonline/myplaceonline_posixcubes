...
if ! cube_file_exists /etc/letsencrypt/live/ ; then
  ...
  /usr/bin/certbot certonly --non-interactive --agree-tos --expand --email contact@myplaceonline.com --dns-digitalocean --dns-digitalocean-credentials ~/.digitalocean.ini --dns-digitalocean-propagation-seconds 30 ${cubevar_app_letsencrypt_tls_domains}
